...
#define MODULE_PRIVATE_DATA  BOOST_PP_CAT(rgauge_data_ist_,ISTANCE_NUM)
...
{
	DEBUG_PRINT_FUNCTION_NAME(NORMAL,"RAINGAUGE INIT ON CHAN "BOOST_PP_STRINGIZE(MODULE_ISTANCE_CHAN) );

	PULSE_INIT(MODULE_ISTANCE_CHAN);

	MODULE_PRIVATE_DATA.maxSlope = 0xFFFF;
	MODULE_PRIVATE_DATA.tips	 = 0;
	return AC_ERROR_OK;
}
#undef MODULE_METHOD_NAME
#endif
...
RET_ERROR_CODE MODULE_METHOD_NAME(void)
{
//	DEBUG_PRINT_FUNCTION_NAME(NORMAL,"RAINGAUGE RESET ON CHAN "BOOST_PP_STRINGIZE(MODULE_ISTANCE_CHAN) );
	MODULE_PRIVATE_DATA.maxSlope = 0xFFFF;
	MODULE_PRIVATE_DATA.tips	 = 0;
	return AC_ERROR_OK;
}
...
{
//	DEBUG_PRINT_FUNCTION_NAME(NORMAL,"RAINGAUGE GETDATA ON CHAN "BOOST_PP_STRINGIZE(MODULE_ISTANCE_CHAN) );
	
	if(!PULSE_IS_EMPTY(MODULE_ISTANCE_CHAN)) 
	{
		PULSE_CHAN_STATISTICS s;
		PULSE_GET_TSTAMP(MODULE_ISTANCE_CHAN,(&s));
		raingauge_tip(&MODULE_PRIVATE_DATA,&s);
	}
	pData->tips = MODULE_PRIVATE_DATA.tips;
	pData->maxSlope = MODULE_PRIVATE_DATA.maxSlope;
