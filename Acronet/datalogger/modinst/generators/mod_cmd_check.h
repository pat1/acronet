...
#undef MODULE_INSPECTION_PHASE
	...
	if(0==strncasecmp_P(pBuf,PSTR(BOOST_PP_STRINGIZE(BOOST_PP_TUPLE_ELEM(2,1,MODULE_CMD_FILTER))),strlen(BOOST_PP_STRINGIZE(BOOST_PP_TUPLE_ELEM(2,1,MODULE_CMD_FILTER))))){
		debug_string_1P(NORMAL,PSTR("FILTERING "BOOST_PP_STRINGIZE(BOOST_PP_TUPLE_ELEM(2,1,MODULE_CMD_FILTER))"\r\n"));
		
		pBuf[lenBuf]=0;
		err = BOOST_PP_TUPLE_ELEM(2,0,MODULE_CMD_FILTER) (pBuf+strlen(BOOST_PP_STRINGIZE(BOOST_PP_TUPLE_ELEM(2,1,MODULE_CMD_FILTER))));
	} else
